JCLASS(Trigger, GetTriggers)
JTYPE(SceneObjectType, SO_Triggers)
JEXPOSE(std::string, uuid, "", jedv_t_hidden, 0, false)
JEXPOSE(bool, hidden, false, jedv_t_boolean, 0, false)
JEXPOSE(std::string, name, "trigger", jedv_t_so_trigger_name, 0, true)
JEXPOSE(bool, skipRendering, false, jedv_t_boolean, 1, false)
JEXPOSE_TRANSFORM(XMFLOAT3, position, ToXMFLOAT3, FromXMFLOAT3, XMFLOAT3(0.0f, 0.0f, 0.0f), jedv_t_float3, 1, false)
JEXPOSE_TRANSFORM_CALLBACK(XMFLOAT3, rotation, ToXMFLOAT3, FromXMFLOAT3, XMFLOAT3(0.0f, 0.0f, 0.0f), jedv_t_float3_angle, 1, false)
JEXPOSE_TRANSFORM(XMFLOAT3, scale, ToXMFLOAT3, FromXMFLOAT3, XMFLOAT3(1.0f, 1.0f, 1.0f), jedv_t_float3, 1, false)
JEXPOSE_TRANSFORM(XMFLOAT4, color, ToXMFLOAT4, FromXMFLOAT4, XMFLOAT4(0.2f, 1.0f, 0.78f, 0.5f), jedv_t_color_float4, 1, false)
JEXPOSE(bool, overrideColor, false, jedv_t_boolean, 1, false)
JEXPOSE(std::string, geometry, "", jedv_t_te_physycgeometry, 1, true)
JEXPOSE_FLAG(objectMask, 0U, GetCollisionMasks, 1, false)
JEXPOSE_FLAG(collisionMask, 0U, GetCollisionMasks, 1, false)
JEXPOSE(std::string, onEnter, "", jedv_t_script, 1, false)
JEXPOSE(std::string, onLeave, "", jedv_t_script, 1, false)
JEXPOSE_VECTOR_TRANSFORM(ScriptBinding, bindings, ToScriptBinding, FromScriptBinding, {}, jedv_t_vector, 1, false)
JEXPOSE(bool, enabled, true, jedv_t_boolean, 1, false)
JEXPOSE(int, countEnter, -1, jedv_t_integer, 1, false)
JEXPOSE(int, countLeave, -1, jedv_t_integer, 1, false)
JEXPOSE_MAP_OBJECT(Controller, controllers, nlohmann::json::object({}), jedv_t_controller_vector, 1, false)
JTRACKUUID(Trigger, Triggers, 0, true)
