...
void EditorDrawFloat(std::string attribute, JObject* json, const char* format, std::function<float(float)> cb)
{
	ImGui::TableNextRow();
...
void EditorDrawFloatArray(std::string attribute, std::vector<JObject*>& json, std::vector<std::string> labels, const char* format)
{
...
void EditorDrawFloatAngleArray(std::string attribute, std::vector<JObject*>& json, std::vector<std::string> labels, const char* format, std::function<void(JObject*)> onUpdate)
{
...
void EditorDrawVector(
	...
	std::function<bool(unsigned int, JUUIDName)> FilterItem,
	std::function<bool(unsigned int, unsigned int, unsigned int, JUUIDName, JUUIDName)> CanSwap
)
{
				...
				drawFarZ, drawZBias](auto& l)
				{
					...
					if (ImGui::BeginTable(tableName.c_str(), 2, defaultTableFlags | ImGuiTableFlags_Hideable))
					{
						...
						if (//type != LT_Directional &&
							type != LT_Ambient) drawPosition(l);
...
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_camera_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_light_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_renderable_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_soundeffect_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_physicscene_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_trigger_name>() { return DrawNonEmptyValue([] {Editor::MarkScenePanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_te_material_name>() { return DrawNonEmptyValue([] {Editor::MarkTemplatesPanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_te_model3d_name>() { return DrawNonEmptyValue([] {Editor::MarkTemplatesPanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_te_renderpass_name>() { return DrawNonEmptyValue([] {Editor::MarkTemplatesPanelAssetsAsDirty(); }); }
template<> JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_te_shader_name>() { return DrawNonEmptyValue([] {Editor::MarkTemplatesPanelAssetsAsDirty(); }); }
...
JEdvEditorDrawerFunction DrawValue<std::string, jedv_t_so_boundary>()
{
	return[](std::string attribute, std::vector<JObject*>& json)
		{
			auto getName = [](JUUID uuid) { return Scene::GetBoundaryName(MAKESUUUID(Editor::currentSceneUnitId, uuid)); };
			DrawResourceSelection(attribute, json, getName, SortUUIDSUNameByName(Editor::currentSceneUnitId, [](auto unit) { return Scene::GetBoundarysIDsNames(unit, false); }), ICON_FA_SNOWMAN, ImGui::OpenSceneObject);
		};
}
...
static std::map<MaterialVariablesTypes, std::function<bool(unsigned int, nlohmann::json&)>> MaterialVariablesTypesDrawers = {
			...
			auto drawPhysicCharacterAttributes = [&]()
				{
					std::string physicObjectUUID = json.at(0)->at(attribute).at(0);
					auto& physicObject = GetPhysicObject(physicObjectUUID);
					if (//!physicObject->controller ||
						physicObject->geometry().empty())
						return;
